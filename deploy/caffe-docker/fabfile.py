...
REPO_HOME="/opt/caffe-docker-workspace"
CONTAINER_NAME = "mohamedezz/jonlong"
...
def add_aliases():
	...
	aliases = ["caffecontainer='sudo GPU=0 nvidia-docker run -v /media/nas:/media/nas -v \$(pwd):/data -P --net=host --workdir=/data -ti --privileged mohamedezz/jonlong bash'"
			]
...
def setup_container():
	...
	# copy docker file
	run('mkdir -p '+REPO_HOME+"/caffe-docker")
	put('Dockerfile',REPO_HOME+"/caffe-docker/")	
	...
	with cd(REPO_HOME+"/caffe-docker/"):
		sudo("docker build -t jonlong .")
		sudo("docker tag -f jonlong "+CONTAINER_NAME)
